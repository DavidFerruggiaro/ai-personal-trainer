...
- `docs/bakeoff_labels/squat_labels_v2.schema.json` + `scripts/squat_label_schema.py`: strict frame-accurate clean-gate label contract with conservative v1 adaptation.
- `scripts/validate_squat_labels.py` + `scripts/audit_clean_rep_evidence.py`: deterministic source/label/pose validation and full-rate-ready observability processing. They classify no clean gate.
...
- The follow-on v2 evidence workflow is implemented without changing that no-go: new labels carry source-frame start/bottom/end events, a standing-reference window, independent pass/fail/unknown required gates, confidence/sufficiency/provenance/capture notes, and source-video SHA-256. New `PoseRunExport` values include an optional source SHA-256 while old exports still decode. The v1 adapter never turns an unmentioned gate into pass. Synthetic 10 FPS fixtures and deterministic processing tests pass; use `docs/bakeoff_results/2026-08-03_clean_rep_evidence/GYM_VIDEO_RUNBOOK.md` for the next clip.
- The old `gym_w_barbell.mov` now has a separate frame-accurate v2 AI-assisted prelabel and a successful local 10 FPS diagnostic replay. All seven reps have complete preferred-side event and standing-window evidence, but the annotation is explicitly `human_verified: false`, contains no failures, and the replay is not an app-owned iPhone export. It tests the workflow without changing the clean-threshold no-go. The replay also fixed rotated-iPhone frame indexing in the runbook.
...
- `TrainerApp` has a compact inline review editor for load, counted reps, and clean reps. Applying edits updates the already auto-saved in-memory set and its stable persisted record; no `Save Set` action or per-rep editing was added.
- Physical review-edit and low-confidence review inspection passed on 2026-08-03.
- M2.13a now persists completed structured `SetResult` records through the isolated `TrainerPersistence` SwiftData package while keeping `TrainerCore` Foundation-only. Stable-ID upsert occurs only after successful full-sequence analysis; review edits replace the same record; confirmed review discard deletes it and empty parents; active/failed/discarded partial captures never persist.
- The persistence schema preserves original and corrected load/count evidence, honest clean provenance, setup override evidence, ordered corrections, per-rep summaries, and available engine/analyzer metadata. Form evidence stays explicitly unavailable. Only opaque artifact IDs exist; no video/image bytes, pose streams, retention controls, history UI, or cloud path was added.
- Ten isolated persistence tests pass, including file-backed reopen from a new container. That is automated lifecycle evidence, not physical restart acceptance; M2.13 remains `in_progress` until an iPhone save/terminate/relaunch inspection also challenges correction and discard durability.
...
9. Verify native changes from the repo root with:
   - `scripts/verify_native_ios.sh` for all five package suites plus both single-architecture workspace Simulator builds.
...
12. Defer M2.14's physical summary check until the next natural multi-set workout. M2.13a is implemented but still needs physical restart acceptance; do not expand it into history UI, video retention, cloud work, coaching, clean-gate invention, or exercise expansion.
...
8. M2.11 is done in both automated and physical checks: correction validation, user-evidence provenance, load carry-forward, and corrected rollback passed. Detailed per-rep editing remains out of scope.
9. M2.5a, M2.7a, and M2.14a from the ranked offline review are done. M2.13a structured persistence is automated but awaits physical restart acceptance. Parent M2.14 still needs the deliberately deferred physical multi-set summary inspection.
