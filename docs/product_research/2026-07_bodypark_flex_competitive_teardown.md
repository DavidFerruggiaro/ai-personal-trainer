...
| 7. Local squat history with next-session continuity | Today’s corrected result disappears, and the next workout starts with no remembered load or evidence of progress. | Flex’s inline prior performance/return loop; BodyPark’s history. | M2.13 persistence is absent; only within-session load carry and a transient summary exist. [R8] [R18] [R21] | Existing persistence boundary planned in M2.13; persist corrected set result, original/provenance, capture confidence, and validated assessment—not pose streams or full videos in structured records. Surface last load and a small squat-only history. | Relaunch/crash/background lifecycle on device; correction/discard durability; unit handling; low-confidence/unavailable interpretation; confirm prior load helps rather than anchoring unsafe progression. | **Medium** | **After** current pass, once review lifecycle is stable |
