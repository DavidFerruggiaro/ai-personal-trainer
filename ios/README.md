...
3. M2.1–M2.4 and M2.7 are complete. M2.5 remains open because the pre-arm weak-setup override proved unreachable for a solo lifter; M2.5b post-arm latched recovery is selected next. M2.6 remains open for duration/failure behavior and an approximately 9-second countdown-to-visible-recording delay measured on device.
...
9. M2.12 is done. Physical zero-rep discard was immediate, detected provisional reps confirmed, and corrected review discard removed the set and restored its corrected load/ordinal. M2.10 remains open until clean-gate evidence exists and low-confidence review is physically reachable.
...
17. The next implementation slice is M2.5b reachable armed weak-setup recovery. After it, diagnose the measured recording-transition delay. Defer M2.14 summary rows/provenance/Done inspection until the next natural multi-set workout.
