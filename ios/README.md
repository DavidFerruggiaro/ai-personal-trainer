...
- New prerecorded exports stream a SHA-256 of the imported source file into optional `source_video.sha256`. Old `PoseRunExport` JSON remains decodable. The content hash pairs Photos imports with v2 labels even when Photos supplies a temporary filename.
...
7. M2.9 is done. On-device Stop removed the camera immediately, processing completed, review opened automatically, and the finalized set required no explicit save action. M2.13a now persists that successful auto-save boundary locally. Processing still has deterministic retry/discard coverage; the fake delay remains removed.
8. M2.10 now shows load × canonical counted reps, provisional/final differences, low-confidence setup labeling, neutral per-rep markers, primary `Next Set`, and secondary discard/end actions. Clean depth/lockout/tempo remain unassessed, so the UI says `Clean reps unavailable` rather than `0 clean reps`.
   - Its offline evidence workflow now includes a machine-readable v2 label schema, conservative v1 adapter, strict source/pose/label validator, standing-window observability audit, and deterministic 10 FPS fixtures. This does not implement a native gate. See `../docs/bakeoff_results/2026-08-03_clean_rep_evidence/GYM_VIDEO_RUNBOOK.md`.
...
11. Review now has a compact inline editor for load/unit, counted reps, and clean reps. Applying edits updates the same in-memory and persisted set, corrected load carries to the next draft, and corrected sets still roll back through review discard and delete the persisted record. There is no explicit Save Set or per-rep editing.
...
13. M2.14a projects corrected, non-discarded completed sets into an immutable Foundation-only session summary. Ending a nonempty workout shows a transient set-by-set projection with counted totals, clean provenance, and low-confidence captures; M2.13a persists the underlying completed set records, but no history UI, issue inference, or mixed-unit volume was added.
...
15. The corrected full harness passes for `PoseCore` (2 XCTest + 3 Swift Testing tests), `SquatAnalysis` (10 tests), `TrainerCore` (56 tests), `TrainerRuntime` (19 tests), `TrainerPersistence` (10 tests), and host-architecture workspace Simulator builds of both `TrainerApp` and `PoseBakeoff`. The v2 evidence workflow also has 21 passing deterministic Python tests. A normal dual-architecture `TrainerApp` build previously exhausted the nearly full internal disk while writing the final universal binary after both architectures compiled and linked.
...
swift test --package-path Packages/TrainerPersistence
```

These deterministic checks prove package contracts, local-store reopen behavior, and Simulator compilation. They do not exercise physical app termination/relaunch, camera capture, MediaPipe landmark generation, CoreMotion, SwiftUI interaction, device performance, or real-world squat accuracy.
